...
`VideoWriter` (`recorder/video_writer.py`) is **eager-launch + restart-on-fast-exit + first-frame-rename**, no probe phase. On `start()` the supervisor thread `Popen`s ffmpeg immediately writing to a placeholder `video/main_<launch_ms>.mp4` (epoch ms at Popen) with `-rw_timeout 5000000` so an unreachable / no-publisher RTMP endpoint makes ffmpeg exit non-zero within ~5s. ffmpeg is also given `-progress pipe:1 -stats_period 0.1`; a daemon thread drains stdout, tracking BOTH the first and latest non-zero `out_time_us=…` values plus the wall ms of the latest one, then back-calculates the first-frame wall ms as `last_observed_at_ms − (last_us − first_us)/1000` and **atomically renames** the file to `main_<first_frame_wall_ms>.mp4` (POSIX rename — ffmpeg's open fd keeps writing through the inode), patching `_segments[-1]["ffmpeg_start_wall_ms"]` so the manifest is accurate. The PTS-delta form is required because `-c copy` forwards the source RTMP PTS straight through; live encoders (DJI Dock 3 included) emit PTS that is their monotonic uptime, not 0-relative — the older `now_ms − out_time_us/1000` form silently assumed `first_frame_pts == 0` and produced wall times **days** before `popen_wall_ms` (a 2026-05-29 production log saw 6.8 days early; regression pinned by `test_pts_not_starting_at_zero_uses_delta_against_first_observation`). Downstream tools rely on `frame_at_PTS_X.recv_ms == ffmpeg_start_wall_ms + X`; the alternative "Popen time" anchor was 1-3s early (RTMP handshake + I-frame wait) and the error compounded across record→play→record cycles. `ffmpeg_popen_wall_ms` is still preserved in `timing.json` and exposed as `manifest.video.popen_at_recv_ms` as a fallback anchor (= "开始拉流" 时刻), unaffected by the rename. If the run lasted < `success_min_seconds` (default **15s**, ≥ 3× rw_timeout) the supervisor treats it as a failed start, deletes the partial mp4 (under whichever name it currently has), pops the segment entry, and retries after `retry_interval_s` (default 2s). If ≥ 15s, supervisor treats it as completed and exits the loop. `main.ffmpeg.log` is opened in **append mode** every attempt with a `=== ffmpeg attempt at wall_ms=… ===` separator so failure traces accumulate. Why eager: the old probe-first design consumed ~5s of source data per probe (RTMP doesn't replay history to new subscribers).
