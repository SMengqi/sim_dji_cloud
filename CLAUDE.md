...
`VideoWriter` (`recorder/video_writer.py`) is **eager-launch + restart-on-fast-exit + first-frame-rename (with popen-fallback)**, no probe phase. On `start()` the supervisor thread `Popen`s ffmpeg immediately writing to a placeholder `video/main_<launch_ms>.mp4` (epoch ms at Popen) with `-rw_timeout 5000000` so an unreachable / no-publisher RTMP endpoint makes ffmpeg exit non-zero within ~5s. ffmpeg also gets `-probesize 20M -analyzeduration 10M` **before `-i`** (input options): DJI Dock 3's H.264-over-RTMP sequence header (SPS/PPS) arrives late/sparsely and overruns ffmpeg's default probe, so the mp4 muxer fails with `dimensions not set / Could not write header (incorrect codec parameters?)` and records an empty shell — even while SRS's WebRTC player shows the stream fine (WebRTC doesn't need dimensions up-front; the mp4 container does). Configurable via `video.ffmpeg_probesize` / `video.ffmpeg_analyzeduration` (note `ffmpeg_extra_args` lands *after* `-i` so it can't fix this); regression `test_probe_args_in_cmd_before_input`. ffmpeg is also given `-progress pipe:1 -stats_period 0.1`; a daemon thread drains stdout, tracking BOTH the first and latest non-zero `out_time_us=…` values plus the wall ms when each arrived, then computes a CANDIDATE first-frame wall ms via `last_observed_at_ms − (last_us − first_us)/1000`. The candidate is then **sanity-checked**: it must lie in `[popen_wall_ms, first_observed_at_ms]` (lower = physical: first frame can't predate Popen; upper = observational: by the time we read the first `out_time_us` line, the first frame is already muxed). If the candidate passes, the file is **atomically renamed** to `main_<first_frame_wall_ms>.mp4` (POSIX rename — ffmpeg's open fd keeps writing through the inode) and `_segments[-1]["ffmpeg_start_wall_ms"]` is patched. If it fails, the placeholder popen-time filename and `ffmpeg_start_wall_ms == ffmpeg_popen_wall_ms` stay put and a `WARNING` is logged with full diagnostics (`first_us / last_us / duration_implied / candidate / popen`). The PTS-delta form is required because `-c copy` forwards source RTMP PTS into the progress stream directly; live encoders (DJI Dock 3 included) emit PTS that is their monotonic uptime, not 0-relative — the older `now_ms − out_time_us/1000` form silently assumed `first_frame_pts == 0` and produced wall times **days** before `popen_wall_ms` (2026-05-29 production log: 6.8 days early; regression pinned by `test_pts_not_starting_at_zero_uses_delta_against_first_observation`). The sanity check is required because some sources/ffmpeg versions report `out_time_us` that is non-monotonic at wall rate altogether — e.g. 2026-06-01 production log saw `first_us=5293000` → `last_us=93270000000` (a 25.9-hour jump) across only ~98s of wall time; `ffprobe` on the same mp4 showed actual frame PTS 0..92.981s (the muxer wrote sane PTS — only `-progress` was lying, likely reporting source DTS or a multi-stream synthetic time). Without the sanity check the formula gave wall = popen − 25.9 hr; with it we degrade to popen-time naming (= "保留开始拉流的时间戳" — exactly the user's original request); regression pinned by `test_implausible_back_calc_falls_back_to_popen`. Downstream tools rely on `frame_at_PTS_X.recv_ms == ffmpeg_start_wall_ms + X`; under sanity-rejection the anchor is RTMP-handshake-early (~1–5 s, consistent and player-correctable) instead of arbitrarily wrong. `ffmpeg_popen_wall_ms` is preserved in `timing.json` and exposed as `manifest.video.popen_at_recv_ms` (= "开始拉流" 时刻), unaffected by either path. On top of all that, `stop()` runs an **ffprobe-based authoritative override** (`_finalize_segment_via_ffprobe`): after ffmpeg exits we read the on-disk mp4's actual container duration via `ffprobe -show_entries format=duration` and compute `first_frame_wall_ms = exit_wall_ms − duration_ms`. ffprobe reads the muxer's real PTS so it sidesteps `-progress out_time_us` entirely; this is the authoritative answer when ffprobe is in PATH (sanity-checked the same way — rejection keeps the in-band anchor). The three-tier chain (popen-time → in-band PTS-delta → ffprobe override) is best-effort upgrade, never downgrade. If the run lasted < `success_min_seconds` (default **15s**, ≥ 3× rw_timeout) the supervisor treats it as a failed start, deletes the partial mp4 (under whichever name it currently has), pops the segment entry, and retries after `retry_interval_s` (default 2s). If ≥ 15s, the run is a **completed segment** — but a mid-flight source drop is NOT "recording done": unless `stop()` has fired the flight is still in progress, so the supervisor finalizes that segment **in place** (joins its progress reader, then ffprobe-anchors it with *its own* exit wall) and **reconnects** — the next ffmpeg run becomes the next segment. This is record-side **segment stitching**: a mid-flight RTMP `Error during demuxing: Input/output error` (DJI Dock 3 does this routinely on a congested link — `speed<1.0x` in the ffmpeg log) no longer ends video for the rest of the flight; recording ends only on `stop()`. Each segment is finalized with its own exit wall (re-running ffprobe later with `now()` would push the first-frame anchor wrong), so `stop()` only re-anchors the segment it itself interrupts. `manifest_video_block` then emits **all** `had_any_frames` segments in `manifest.video.segments[]` with per-segment `start_ms`/`end_ms`/`started_at_recv_ms`/`popen_at_recv_ms`/`duration_ms`; the top-level `file`/`started_at_recv_ms`/`popen_at_recv_ms` stay pointed at the **first** segment for backward-compat. **Playback still pushes only the first segment** (`player/video_pusher.py`) — multi-segment replay is a follow-up. Regressions: `test_reconnects_after_long_run_self_exit`, `test_manifest_block_multi_segment`. A ≥15s run only counts as a segment if it actually produced frames (`had_any_frames`, set when the progress reader saw a non-zero `out_time_us` OR ffprobe read duration > 0). A ≥15s run with **no frames is treated as a failed start** — deleted, segment dropped, retried — NOT kept as an empty segment. This matters with `-reconnect` on http sources: an unreachable / not-yet-publishing endpoint makes ffmpeg retry the open and burn past 15s before exiting `Error opening input: Connection timed out`; that's no recording, so the reconnect loop must not accumulate phantom no-frame segments + junk files (regression `test_no_frame_self_exit_is_failed_start_not_phantom_segment`). The ONE no-frame case that IS kept on disk for diagnostics is a segment that `stop()` itself interrupts (terminal, not in a retry loop) — `manifest_video_block` still filters it out of `manifest.video` (`had_any_frames=False`). Real-machine repro (2026-06-04): RTMP handshake succeeded + stream metadata parsed, then `Error during demuxing: Input/output error` immediately — ffmpeg sat 16s then exited; mp4 was 818 B (moov header only). Pre-fix, this segment was written into `manifest.video` and dashboard tried to replay an empty file. Regression pinned by `test_empty_shell_mp4_skipped_in_manifest` + `test_sanity_rejected_back_calc_still_keeps_manifest_video` (the latter is the "had frames but anchor unreliable" path — sanity-rejected back-calc but `had_any_frames=True` → manifest still gets video with popen-time anchor). `main.ffmpeg.log` is opened in **append mode** every attempt with a `=== ffmpeg attempt at wall_ms=… ===` separator so failure traces accumulate. Why eager: the old probe-first design consumed ~5s of source data per probe (RTMP doesn't replay history to new subscribers).
