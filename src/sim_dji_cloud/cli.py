...
@click.option("--log-dir", default="./logs", type=click.Path(),
              help="HTTP 控制 play 子进程的 pid 文件 + 日志目录，默认 ./logs（跟 run.sh LOG_DIR 共享）")
@click.option("--default-video-push-url", default="", type=str,
              help="dashboard 切飞行时默认带的视频推流 URL（如 rtmp://srs/live/livestream）；"
                   "留空则切飞行不推视频")
def dashboard_cmd(mqtt_url: str, host: str, port: int, ws_push_interval_ms: int,
                  ...
                  video_url: str | None, recordings_root: str, log_dir: str,
                  default_video_push_url: str) -> None:
